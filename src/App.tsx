...
export default function App() {
  ...
  const [loadingStep, setLoadingStep] = useState(0); // simulation loading percentage
  const [loadingText, setLoadingText] = useState('Menginisialisasi analisis...');
  const [screen, setScreen] = useState<ScreenName>('login');
  
  // State variables synchronized
  const [currentUser, setCurrentUser] = useState<User>(DEFAULT_USER);
  const [history, setHistory] = useState<AssessmentResult[]>(DEFAULT_HISTORY);
  const [latestResult, setLatestResult] = useState<AssessmentResult | null>(DEFAULT_HISTORY[0]); // most recent is hist_1 (first in DEFAULT_HISTORY, wait, let's look at DEFAULT_HISTORY: we put hist_1 which is Low, hist_2 is High, hist_3 is Medium, hist_4 is Low. Actually, let's select history[0] as our default start latestResult)
  ...
  // Sinkronisasi status login Google (Firebase Auth) -> state lokal & redirect otomatis
  useEffect(() => {
  ...
  // Trigger loading screen and calculate results
  const handleAssessmentSubmit = (input: AssessmentInput) => {
    ...
    // Simulate multi-step advanced neural tree calculating
    ...
    const timer = setInterval(() => {
      if (currentIdx < loadingIntervals.length) {
        ...
        // Temporarily set it so they view it, but wait! We ONLY save it when they click "Simpan Hasil" on recommendations screen!
        // This is a fabulous UX so they can adjust recommendation check boxes first or see the SHAP explanation first.
  ...
  // Save recommendation checklist updates & save to list
  ...
  // Quick switch screen panel (extremely helpful helper tools!)
  ...
  // Load Presets instantly to demonstrate different risk profiles
  const handleLoadPreset = (profile: 'low' | 'high' | 'medium') => {
    ...
    setLatestResult(result);
    // Add to history directly to make chart reflect the state
    setHistory(prev => [result, ...prev]);
    setScreen('result');
  };

  // Render correct active screen inside mock device
  const renderSimulatedApp = () => {
    switch (screen) {
      ...
      case 'loading':
        return (
          <div className="flex flex-col items-center justify-center p-6 text-center space-y-6 h-full bg-slate-50">
            {/* Real calculation loading animated spinner */}
            ...
            <div className="space-y-2 max-w-xs">
              ...
              <p className="text-xs text-gray-400 font-mono tracking-tight h-8">
                {loadingText}
              </p>
            </div>
      ...
      default:
        return <DashboardScreen currentUser={currentUser} latestResult={latestResult} onNavigate={(tgt) => setScreen(tgt)} onSelectHistoryItem={setLatestResult} />;
    }
  };

  // Bottom Device Tabs rendered inside appropriate parent mock-views (Home, Assesmen, History, Profil)
  ...
  return (
    <div className="min-h-screen bg-elegant-bg text-slate-300 flex flex-col font-sans" id="app-root-container">
      {/* Top Main branding panel matching LUMINA design header */}
      <header className="bg-elegant-panel border-b border-white/5 py-4 px-6 flex items-center justify-between shadow-lg">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-[10px] font-mono bg-white/5 text-elegant-gold px-2.5 py-1 rounded-lg border border-white/10 font-bold uppercase tracking-wider">
            STATUS: SECURE LIVE
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center justify-items-center">
        {/* Left pane: Educational documentation and developer tools */}
        <section className="lg:col-span-7 space-y-6 w-full text-left order-2 lg:order-1">
          <div className="space-y-3">
            ...
            <h2 className="text-3xl md:text-5xl font-extrabold text-white tracking-tight leading-tight font-serif-elegant">
              Analisis Risiko Burnout <span className="bg-gradient-to-r from-elegant-gold via-elegant-gold-light to-white bg-clip-text text-transparent">Premium</span>
            </h2>
            <p className="text-sm text-slate-400 leading-relaxed max-w-2xl">
              Gunakan simulator genggam interaktif di sebelah kanan untuk mengevaluasi parameter keseimbangan kerja karyawan. Sistem menghitung tingkat probabilitas kelelahan menggunakan model simulasi kontribusi parameter global berkode <b className="text-elegant-gold">SHAP (Shapley Additive exPlanations)</b>.
            </p>
          </div>

          {/* Interactive Profile Preset quick-loaders */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              >
                <div className="flex justify-between items-center w-full">
                  <span className="group-hover:text-white transition-colors">Profil Khawatir (Rendah)</span>
                  <CheckCircle className="w-3.5 h-3.5 text-emerald-500" />
                </div>
                <span className="text-[10px] text-slate-400/80 mt-1">Beban rukun + WFH aktif</span>
              </button>
          ...
          {/* Quick-routing developer screen shortcuts */}
        ...
        {/* Right pane: Handheld Smartphone emulator device mockup frame styled with gold offsets */}
        <section className="lg:col-span-5 w-full flex items-center justify-center order-1 lg:order-2">
          {/* Smartphone Frame Outer Cover styled meticulously with elegant dark tone */}
          <div className="relative w-full max-w-[375px] h-[780px] bg-[#0d0e12] rounded-[55px] p-3 border-4 border-white/10 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.9)] overflow-hidden ring-4 ring-[#161922] flex flex-col">
            ...
            {/* Elegant Golden Side Rim Highlights */}
            ...
            {/* Dynamic Status bars (Speaker Notch, Time, Battery) */}
            ...
            <div className="flex-1 bg-elegant-bg rounded-[42px] overflow-hidden flex flex-col relative z-20 shadow-inner">
              {/* Status Indicators below top Notch */}
              ...
              {/* Core Active screen navigation views */}
              ...
              {/* Bottom persistent Apple bar matching screenshot styles */}
                    ...
                    return (
                      <button
                        ...
                        className={`flex flex-col items-center justify-center space-y-1 transition text-[10px] font-bold tracking-tight w-14 cursor-pointer ${
                          isActive ? 'text-elegant-gold font-extrabold scale-102' : 'text-slate-500 hover:text-slate-300'
                        }`}
                      >
                        <TabIcon className={`w-4.5 h-4.5 ${isActive ? 'stroke-[2.5] text-elegant-gold' : 'stroke-[1.8] text-slate-500'}`} />
                        <span>{tab.label}</span>
                      </button>
                    );
                  })}
              ...
              {/* Bottom pill representation home indicator */}
      ...
      {/* Unified simple branding footer */}
